...
def xgbCV(X_train,X_test):
    ...
    model = xgb.train(params,train,num_boost_round=5000,evals=watchlist,early_stopping_rounds=500)
...
def sub(X_train,X_test,best_int):
    ...
    watchlist = [train]
    model = lgb.train(lgb_params, train, num_boost_round =best_int ,valid_sets = watchlist)    
    ...
    sub.to_csv('./20180511_gene_lgb_before.txt',sep=" ",index=False)
    return model
...
def save_feature_importance(feat_imp):
    ...
    feat.to_csv('../feature/gene/feature_importance_gene_0507.csv',sep=",",index=False)
...
#    gc.collect()
    ...
    #5.删除特征 
    drop_list=['context_timestamp','user_id','shop_id','item_id','property_0','property_1','item_category_list','item_brand_id'] #item_brand_id , item_category_list
    ...
    #1. 得到测试集
    test  = data[(data['day'] == 7) & (data['hour'] == 11)] 
    test_online = pd.read_csv('../training_data/round2_ijcai_18_test_a_20180425.txt',sep=' ')
    train_ins = list(data.instance_id) ; test_ins=list(test.instance_id)+list(test_online.instance_id)
    train_ins = list(set(train_ins).difference(set(test_ins)))
    #2. 得到训练集
    train = data[data['instance_id'].isin(train_ins)] 
    del test_online ; del data ; gc.collect()

    print('train shape: ',train.shape,'  test shape: ',test.shape)
    #3. 线下训练
    feat_imp,Pre_test,loss,best_int=xgbCV(train,test)
